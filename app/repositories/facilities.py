...
class RoomsFacilitiesRepository(BaseReposirory):
    ...
    async def set_facilities_by_room(self, room_id: int, facility_ids: list[int]):
        get_facilities_ids = select(RoomsFacilitiesOrm.facility_id).filter_by(
            room_id=room_id
        )
        ...
        if ids_to_remove:
            delete_stmt = delete(RoomsFacilitiesOrm).where(
        ...
        if ids_to_add:
            insert_stmt = insert(RoomsFacilitiesOrm).values(
